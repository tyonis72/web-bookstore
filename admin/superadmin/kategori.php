        ...
        <main class="flex-1 p-8 lg:p-12">
            ...
            <div class="flex justify-between items-end mb-10">
                <div>
                    ...
                    <p class="text-indigo-300/50 text-xs font-bold uppercase tracking-[0.3em] mt-2">Klasifikasi Data
                        Katalog Utama</p>
                </div>
            ...
            <?php if (isset($_GET['error'])): ?>
                <div
                    class="mb-6 p-4 glass-card border-red-500/30 text-red-400 rounded-2xl flex items-center gap-3 text-xs font-black uppercase tracking-widest">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    Duplicate detected: Nama kategori sudah digunakan!
                </div>
            <?php endif; ?>
            ...
            <div class="glass-card rounded-[2.5rem] overflow-hidden shadow-2xl">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            class="bg-white/5 border-b border-white/5 text-gray-500 text-[10px] uppercase font-black tracking-[0.2em]">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        while ($row = mysqli_fetch_assoc($kategori)): ?>
                            <tr class="hover:bg-white/5 transition-all group">
                                <td class="p-6">
                                    <span
                                        class="text-indigo-400 font-mono font-bold">#<?= str_pad($no++, 2, '0', STR_PAD_LEFT) ?></span>
                                </td>
                                ...
                                <td class="p-6 text-right">
                                    <div class="flex justify-end gap-3">
                                        <button onclick="openEditModal('<?= $row['id'] ?>','<?= $row['nama'] ?>')"
                                        ...
                                        <a href="../../controllers/KategoriController.php?delete=<?= $row['id'] ?>"
                                            onclick="return confirm('Hapus klasifikasi ini? Tindakan ini tidak dapat dibatalkan.')"
    ...
    <div id="modalBackdrop"
        ...
        <div id="addModal" class="hidden glass-card w-full max-w-md rounded-[3rem] p-10 relative">
            ...
            <form action="../../controllers/KategoriController.php" method="POST" class="space-y-6">
                <div>
                    <label
                        class="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-2 block ml-1">Kategori
                        Baru</label>
                    <input type="text" name="nama" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-emerald-500 text-white font-bold transition-all"
                        placeholder="Contoh: Fiksi, Sains, Sejarah">
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" onclick="closeModal()"
                        class="flex-1 px-6 py-4 bg-white/5 hover:bg-white/10 text-gray-400 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Abort</button>
                    <button type="submit" name="add"
                        class="flex-2 px-10 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl shadow-xl transition-all text-[10px] font-black uppercase tracking-widest">Execute
                        Save</button>
                </div>
            </form>
        </div>
        ...
        <div id="editModal" class="hidden glass-card w-full max-w-md rounded-[3rem] p-10 relative">
            ...
            <form action="../../controllers/KategoriController.php" method="POST" class="space-y-6">
                ...
                <div>
                    <label class="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-2 block ml-1">Nama
                        Klasifikasi</label>
                    <input type="text" name="nama" id="editNama" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl p-4 focus:outline-none focus:border-indigo-500 text-white font-bold transition-all">
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" onclick="closeModal()"
                        class="flex-1 px-6 py-4 bg-white/5 hover:bg-white/10 text-gray-400 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Cancel</button>
                    <button type="submit" name="edit"
                        class="flex-2 px-10 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl shadow-xl transition-all text-[10px] font-black uppercase tracking-widest">Update
                        Label</button>
                </div>
            </form>
        </div>
